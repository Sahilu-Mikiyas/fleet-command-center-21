...
const highlights = [
  { icon: Truck, title: 'Smart Fleet Visibility', body: 'Monitor shipments, vehicles, and delivery milestones from one elegant command surface.' },
  { icon: Zap, title: 'Fast Dispatch Workflows', body: 'Create orders, match transporters, and orchestrate execution with streamlined operations.' },
  { icon: Shield, title: 'Secure Operational Control', body: 'Role-aware permissions keep workflows controlled, safe, and audit-ready.' },
];
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      >
        <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
          <Link to="/" className="text-lg font-bold font-display tracking-tight">FleetCommand</Link>
      ...
      <section className="relative overflow-hidden">
        <div
          ...
          style={{
            backgroundImage:
              "linear-gradient(120deg, hsl(20 20% 10% / 0.82), hsl(20 20% 10% / 0.52)), url('https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&w=1800&q=80')",
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-6 py-24 md:py-32">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="max-w-3xl">
            <p className="inline-flex items-center rounded-full border border-primary-foreground/30 bg-primary-foreground/10 px-4 py-1 text-xs uppercase tracking-[0.18em] text-primary-foreground">
              FleetCommand Platform
            </p>
            <h1 className="mt-5 text-4xl font-bold font-display leading-tight text-primary-foreground md:text-6xl">
              Professional Logistics Coordination, Designed for Speed
            </h1>
            <p className="mt-5 max-w-2xl text-base text-primary-foreground/90 md:text-lg">
              A modern operations platform for shippers, transporters, brokers, and fleet teams — from order creation to live tracking and secure payment.
            </p>

            <div className="mt-8 flex flex-wrap gap-3">
              <Button asChild className="h-12 px-6 gradient-primary text-primary-foreground shadow-lg shadow-primary/20">
                <Link to="/login">
                  Login to Dashboard <ArrowRight className="ml-2 h-4 w-4" />
                </Link>
              </Button>
              <Button asChild variant="secondary" className="h-12 px-6 border border-primary-foreground/25 bg-primary-foreground/15 text-primary-foreground hover:bg-primary-foreground/25">
                <Link to="/pay-order">
      ...
      <section id="features" className="mx-auto max-w-7xl px-6 py-16">
        ...
        >
          <h2 className="text-3xl font-bold font-display tracking-tight">Everything Important on the Homepage</h2>
          <p className="mt-2 text-sm text-muted-foreground">Purpose-built sections, refined visual hierarchy, and smooth interactions.</p>
        </motion.div>
        <motion.div
        ...
        >
          {highlights.map((h, i) => (
            <motion.div key={h.title} initial={{ opacity: 0, y: 14 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -4 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
              <Card className="h-full border-border bg-card shadow-card transition-all hover:shadow-card-hover">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-10">
        ...
        >
          {stats.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="rounded-xl bg-muted/40 py-3 text-center">
      ...
      <section id="about" className="mx-auto grid max-w-7xl items-center gap-8 px-6 pb-16 lg:grid-cols-2">
        <motion.div initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
          ...
          <p className="mt-3 text-muted-foreground">
            We build logistics products that combine enterprise-grade control with tasteful design — giving teams confidence, speed, and clarity across every shipment lifecycle.
          </p>
        ...
        <motion.div initial={{ opacity: 0, x: 16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative">
          ...
          />
          <div className="absolute -bottom-4 -left-4 rounded-xl border border-border bg-card/95 px-4 py-3 shadow-card backdrop-blur">
